...
namespace lime {
	...
	void SDLRenderer::CreateMovieTexture(unsigned int w, unsigned int h){
		
		if(!sdlMovie){
		
			sdlMovie = SDL_CreateTexture(sdlRenderer,SDL_PIXELFORMAT_IYUV,SDL_TEXTUREACCESS_STREAMING,w,h);
	...
	void SDLRenderer::DestroyMovieTexture(){
		
		if (sdlMovie) {
			
			SDL_DestroyTexture(sdlMovie);
